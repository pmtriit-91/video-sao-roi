...
export const CenterTypography: React.FC<CenterTypographyProps> = ({
  ...
  return (
    ...
    >
      {/* 1. Monogram Logo (H lồng T - Chuẩn Royal Typography Khối Dày & Ánh Kim Metallic Sheen) */}
      <div
        style={{
          marginBottom: height * 0.022,
          opacity: 0.98,
          filter: `drop-shadow(0 0 ${12 * glowBreath}px ${palette.textGlow}) drop-shadow(0 0 ${28 * glowBreath}px ${palette.secondary})`,
          willChange: "filter",
        }}
      >
        {config.monogramSvgPath ? (
          ...
          (() => {
            // Chu kỳ quét sáng: 300 frames (5 giây ở 60fps) - chia hết cho 2400 frames (đúng 8 chu kỳ lặp hoàn hảo)
            const SHIMMER_CYCLE = 300;
            const SWEEP_DURATION = 90; // 1.5 giây quét dải sáng chéo qua mặt kim loại
            ...
            // Easing mượt mà không gấp khúc
            const sweepEase = isSweeping
              ? sweepProgress < 0.5
                ? 4 * sweepProgress * sweepProgress * sweepProgress
                : 1 - Math.pow(-2 * sweepProgress + 2, 3) / 2
              : 0;

            // Quét chéo từ góc trên-trái sang góc dưới-phải (viewBox x: 40 -> 350)
            const shimmerX = -120 + sweepEase * 500;

            // Nhịp lóe sáng kim cương 4 cánh (Diamond Glint Flare) khi dải sáng lướt qua chóp chữ
            const glintStart = 38;
            const glintEnd = 54;
            const isGlinting = cycleFrame >= glintStart && cycleFrame <= glintEnd;
            const glintT = isGlinting ? (cycleFrame - glintStart) / (glintEnd - glintStart) : 0;
            const glintIntensity = isGlinting ? Math.sin(glintT * Math.PI) : 0;
            const glintOpacity = glintIntensity * 0.95;
            const glintScale = 0.5 + 0.6 * glintIntensity;

            // Màu metallic sheen tương thích theo palette chủ đề cưới
            ...
            return (
              ...
              >
                <defs>
                  {/* Gradient ánh kim (Metallic Sheen) mượt mà, phẳng mịn, tuyệt đối không bị xướt / vằn sọc */}
                  <linearGradient id="monogramMetallic" x1="0%" y1="0%" x2="20%" y2="100%">
                    {isGold ? (
                      ...
                      /* Bạch kim / Bạc ánh kim (Platinum / Silver Diamond) mịn màng đồng điệu trần sao */
                      <>
                        <stop offset="0%" stopColor="#FFFFFF" />
                        <stop offset="25%" stopColor="#EFF4F9" />
                        <stop offset="55%" stopColor="#CAD7E4" />
                        <stop offset="85%" stopColor="#9FB0C2" />
                        <stop offset="100%" stopColor="#8596A8" />
                      </>
                    )}
                  </linearGradient>

                  {/* Dải quét sáng gương êm dịu (Specular Shimmer Sweep) */}
                  <linearGradient id="monogramShimmer" x1="0%" y1="0%" x2="100%" y2="0%">
                  ...
                  <mask id="monogramMask">
                    <g transform="translate(-48.000000, 854.000000) scale(0.100000, -0.100000)" fill="#FFFFFF">
                      ...
                      <path d="M7320 4640 c0 -60 3 -70 18 -71 174 -7 345 -22 456 -38 189 -29 498 -119 645 -188 62 -29 115 -53 117 -53 2 0 49 -30 106 -66 202 -131 370 -297 504 -497 57 -85 108 -137 137 -137 19 0 47 43 47 73 0 63 -21 104 -123 234 -252 322 -575 557 -932 679 -247 85 -544 134 -812 134 -163 0 -163 0 -163 -70z" />
                ...
                {/* Khối chữ lồng H - T chuẩn Royal với chất liệu Gradient Platinum / Silver sang trọng */}
                ...
                {/* Hiệu ứng nhịp lướt sáng gương (Light Sweep Shimmer) chéo qua mặt kim loại theo chu kỳ */}
                {isSweeping && (
                  <g mask="url(#monogramMask)">
                    <rect
                      x={-280 + sweepEase * 1400}
                      y={-50}
                      width={180}
                      height={900}
                      transform={`rotate(25, ${-280 + sweepEase * 1400 + 90}, 360)`}
                      fill="url(#monogramShimmer)"
                    />
                  </g>
                )}

                {/* Điểm xuyết tia lóe sáng kim cương 4 cánh (Diamond Glint Flare) khi dải sáng lướt qua */}
                {isGlinting && glintOpacity > 0.01 && (
                  <g
                    transform={`translate(520, 260) scale(${glintScale * 1.8})`}
                    opacity={glintOpacity}
                    style={{ pointerEvents: "none" }}
                  >
                    <circle cx="0" cy="0" r="14" fill="rgba(255, 255, 255, 0.4)" />
                    <polygon points="-24,0 0,-1.8 24,0 0,1.8" fill="#FFFFFF" />
                    <polygon points="0,-24 -1.8,0 0,24 1.8,0" fill="#FFFFFF" />
                    <polygon points="-12,-12 0,-1 12,12 0,1" fill="rgba(255, 255, 255, 0.75)" />
                    <polygon points="-12,12 -1,0 12,-12 1,0" fill="rgba(255, 255, 255, 0.75)" />
